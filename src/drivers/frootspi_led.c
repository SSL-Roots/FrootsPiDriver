...
extern int mcp23s08_write_gpio(const unsigned char gpio_num, const unsigned char value);
...
int register_led_dev(void)
{
	...
	// 動的にメジャー番号を確保する
	retval = alloc_chrdev_region(&dev, LED_BASE_MINOR, LED_MAX_MINORS, LED_DEVICE_NAME);
	if (retval < 0) {
		// 確保できなかったらエラーを返して終了
	...
	for (int i = 0; i < LED_MAX_MINORS; i++){
		...
		device_create(led_class, NULL, MKDEV(led_major, LED_BASE_MINOR + i),
			NULL, "%s%u", LED_DEVICE_NAME, i);
	}
...
failed_class_create:
	unregister_chrdev_region(MKDEV(led_major, LED_BASE_MINOR), LED_MAX_MINORS);
	return retval;
}
