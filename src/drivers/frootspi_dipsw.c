...
static ssize_t dipsw_read(struct file *filep, char __user *buf, size_t count,
			     loff_t *f_pos)
{
...
int register_dipsw_dev(void)
{
	...
	// 動的にメジャー番号を確保する
	retval = alloc_chrdev_region(&dev, DIPSW_BASE_MINOR, DIPSW_MAX_MINORS, DIPSW_DEVICE_NAME);
	if (retval < 0) {
		// 確保できなかったらエラーを返して終了
	...
	for (int i = 0; i < DIPSW_MAX_MINORS; i++){
		...
		device_create(dipsw_class, NULL, MKDEV(dipsw_major, DIPSW_BASE_MINOR + i),
			NULL, "%s%u", DIPSW_DEVICE_NAME, i);
	}
...
failed_class_create:
	unregister_chrdev_region(MKDEV(dipsw_major, DIPSW_BASE_MINOR), DIPSW_MAX_MINORS);
	return retval;
}
...
void unregister_dipsw_dev(void)
{
	...
	for (int i = 0; i < DIPSW_MAX_MINORS; i++){
		device_destroy(dipsw_class, MKDEV(dipsw_major, DIPSW_BASE_MINOR + i));
